...
| `20260818-viralrecon-sample01-realsample` | nf-core/viralrecon against one real sample from the pipeline's own protocol-confirmed `test_full.config` cohort — `--skip_assembly true`, explicit `--fasta`/`--gff`/`--primer_bed` instead of `--genome` shorthand, and three separate runtime-fetch/rate-limit/TLS environment findings (pangolin/nextclade/freyja) routed around via pre-fetched local DBs; a genuine low-viral-titer FAIL result on consensus completeness, reported as measured, not re-picked for a nicer number |
| `20260820-spatialaxe-testprofile-procurement` | nf-core/spatialaxe CI-fixture run — FIRST spatial/imaging-based pipeline stocked here; `-stub-run` **passes cleanly** (no waiver, unlike 10 of the 11 prior new-pipeline procurements), full test profile clean, `--mode coordinate` (Proseg) |
| `20260820-spatialaxe-realsample` | nf-core/spatialaxe against a real, tiny (23-cell) 10x Genomics Xenium "tiny_outs" bundle — the samplesheet's `bundle` column is a DIRECTORY, not a FASTQ path, and the pipeline's own post-staging bundle-content check (16 required files) sits on top of an otherwise-live JSON schema, a shape not seen in any prior stocked pipeline here |
