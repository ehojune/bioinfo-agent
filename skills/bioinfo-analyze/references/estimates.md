...
| **pacbio-hifi-wgs** (in-repo, subreads-entry smoke, `-profile test,docker`) | 1 movie, `alz.1perc.subreads.10000.bam` (same real 48 MB CI subset the isoseq rows use, streamed over https), `ccs_chunks 2`, chr19-only ref, callers skipped (RNA subreads — mechanics gate only) | **~2 min** compute (CCS 23 s/chunk ×2 parallel, pbmerge 6 s, pbmm2 1 s, QC+MultiQC seconds) | **226 MB** | **16 MB** | Measured 2026-08-20 (`docs/examples/20260820-pacbio-hifi-wgs-validation/`) |
| **pacbio-hifi-wgs** (in-repo, full caller chain, one real WGS slice) | HG002 GIAB Sequel II HiFi chr20:1–3 Mb (11,004 reads / 124 MB fastq.gz, S3 range-fetch), `hifi_fastq` entry, chr20-only GRCh38 ref, DeepVariant+Clair3(hifi_sequel2)+pbsv+WhatsHap+QC all on | **~3 min** compute — DeepVariant 37 s / **peak 16.1 GB** (model-load floor, roughly input-size-independent), Clair3 48 s / 2.7 GB, pbmm2 8 s, everything else seconds | **610 MB** | **237 MB** | Measured 2026-08-20 (same record). **Not a WGS estimate**: a full 30× human genome multiplies read volume ~1000×; DeepVariant/Clair3 scale with read volume (expect many hours), `pbsv call` RAM grows to tens of GB (config provisions 64 GB), pbmm2 full-genome `.mmi` build adds ~10–15 GB RAM once per run. Measure the first real full-genome run before quoting numbers — measured since: see the whole-genome rows below |
| **pacbio-hifi-wgs** somatic (in-repo, `--somatic_input`, one real slice) | GIAB HG008-T 116x vs HG008-N-P 35x, GRCh38-GIABv3 BAMs sliced to chr13:82–86 Mb, `--deepsomatic_regions` = that window, 16 CPU | **1 m 46 s** DEEPSOMATIC wall (make_examples ≈ 266 CPU-s, call_variants ≈ 204 CPU-s), 159 s whole run | — | **17.8 GB** peak RSS | Measured 2026-09-25 (`docs/examples/20260925-pacbio-somatic-cpu-validation/`). **Not a WGS estimate**: one 4 Mb window need not represent the genome, and memory was not measured above 16 shards. Measure the first whole-genome pair before quoting a schedule — measured since: see the whole-genome rows below |
| **pacbio-hifi-wgs** germline, whole genome (in-repo, GIAB) | 45 GIAB HiFi datasets HG001–HG009 (49 runs), `hifi_fastq`/`hifi_bam`, 43–166 GiB input, GRCh38 no-alt; DeepVariant+Clair3+pbsv+WhatsHap+QC all on | **4.7 – 11.7 h** per dataset at 30 CPU (Nextflow `local` executor inside one SGE job on a 64-core node) | not measured | not measured | Measured 2026-08-21 → 09-24 (`docs/examples/20260926-pacbio-hifi-wgs-giab-wholegenome/`), pipeline at `48ec463`. Job maxvmem 38–80 GB (SGE virtual memory, whole job) |
| **pacbio-hifi-wgs** somatic, whole genome (in-repo, GIAB) | 13 HG008 tumour–normal pairs from aligned BAMs, tumour 29–79x, normal 35–68x | **2.4 – 3.4 h** per pair at 60 CPU (`--num_shards=60`, one node per pair) | not measured | not measured | Measured 2026-09-25 (same record), pipeline at `f2de95e`. Job maxvmem 200–258 GB virtual — near a 251 GB node's RAM; resident memory not measured, one shard count only |
