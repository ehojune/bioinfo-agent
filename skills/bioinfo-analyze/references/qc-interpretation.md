...
| Spike-in fraction (CUT&RUN, E. coli) | cutandrun spike-in scale factors | 0.1–10% of reads | <0.05% | ~0% | Below ~0.05% the scale factor is estimated from too few reads and normalisation adds noise instead of removing it. Report the raw spike-in counts, not just the factor. **Only exists under `--normalisation_mode Spikein`** — a `CPM` run runs no spike-in alignment at all (measured, `pipeline-selection.md` §4.8), so this row is not applicable rather than failed |
