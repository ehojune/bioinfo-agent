...
schema_cols() {  # usage: schema_cols rnaseq
  local f="${NXF_ASSETS:?}/nf-core/$1/assets/schema_input.json"
...
**Revision provenance for everything below.** These tags are what the tables were written
against, from knowledge current to mid-2026. Confirm each tag resolves with
`nextflow info nf-core/<pipeline>` before pinning it in a run plan.

| pipeline | table written against |
|---|---|
| rnaseq | 3.18.0 (columns unchanged across 3.14–3.19) |
| sarek | 3.5.1 |
| fetchngs | 1.12.0 |
...
Sheet says `data/S1_R1.fastq.gz`, you launched from `/home/ehojune/runs`.
...
Save as `$BIOINFO_HOME/scripts/check-samplesheet.sh`, `chmod +x`. Read-only, safe to re-run.
...
# and add to .gitattributes:   *.sh text eol=lf
#
...
# ---- 4. mate orientation and pairing ---------------------------------------
I1=$(colidx fastq_1); I2=$(colidx fastq_2)
if [[ -n "$I1" && -n "$I2" ]]; then
  while IFS=$'\t' read -r R1 R2; do
    [[ -n "$R2" && -r "$R1" && -r "$R2" ]] || continue
    # one decompression pass each: first two lines are enough (head closes the pipe)
    ...
    # 10x sanity: the barcode+UMI read must be much SHORTER than the cDNA read
    L1=${#S1}; L2=${#S2}
    (( L1 > 0 && L2 > 0 && L1 > L2 * 2 )) \
...
```bash
scripts/check-samplesheet.sh --deep /work/runs/2026-07-28-rnaseq/samplesheet.csv
```
