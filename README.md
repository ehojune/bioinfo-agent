...
<p align="center">
  ...
  <a href="#예시-sarek-germline-변이-검출">sarek 예시</a>
</p>
...
**개인 워크스테이션에서 돌고, 공용 서버나 HPC 클러스터에서도 돈다.** 달라지는 건 컨테이너 엔진과
executor 설정뿐이다.

**하지 않는 것**: 생물학적 해석. `NF1 gene isoform NF1-201의 발현 percentage는 17%` 까지가 이 에이전트의 일이고,
...
끝이다. 이제 데이터 경로와 하고 싶은 분석을 말하면 파이프라인 선정, 샘플시트, 소요 추정,
계획서까지 바로 나온다.
...
서버 / 클러스터)를 먼저 판별한 뒤 필요한 스크립트만 골라 돌린다. 무엇이 준비됐고 무엇이 빠졌는지도
...
Y-STR 프로젝트에서 실제로 오간 세션이다. 읽기 좋게 줄바꿈만 다듬었다.

눈여겨볼 곳은 **묻지 않은 것을 먼저 확인했다는 점**이다. `fastq로 WGRS 분석해줘` 한 줄에서
출발했는데, 디렉토리를 훑다가 예전에 손으로 돌려둔 `sorted.bam` 을 발견하고 헤더를 열어 정렬
기준까지 대조했다. 그래서 FASTQ부터 다시 정렬하는 90~270시간을 건너뛰고 `--step markduplicates`
로 재진입하는 안이 나왔다. 그리고 **거기서 멈추고 물어본다.** 산출물을 어디까지 낼지는 사람이
정할 몫이다.
...
써보면 시키지 않은 조사를 먼저 한다. 그것은 실제 분석 환경에서 생길 수 있는 많은 변수 (주로 human error) 를 통제하기 위함이다. 

해당 작업은 별도의 **에이전트 루프** 에 의해 진행된다.

**에이전트**는 자기 컨텍스트 창과 도구 권한(`Bash`, `Read`, `Grep`…)이 따로 있는 별도 인스턴스다.
한 번 답하고 끝나지 않고 **관찰 → 판단 → 실행**을 목표에 닿을 때까지 반복한다. 파이프라인이 죽으면
로그를 읽고 원인을 짚어 파라미터를 고친 뒤 `-resume` 하는 것까지 사람 손을 안 탄다.
...
파이프라인이 죽으면 스킬의 실패 유형 문서로 진단하고, 알아낸 해법을 다시 스킬에 적어 개인화한다.
...
파일을 직접 세어보고 없는 레퍼런스를 짚은 뒤, 시간과 디스크를 계산해 계획서를 낸다.
